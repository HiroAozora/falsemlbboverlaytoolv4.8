...
// ============================================================
// ITEM LOOKUP (untuk display overlay)
// ============================================================
let itemLookupDisplay = {};
let nameToIconDisplay = {}; // { equipname: equipicon_url } — CDN fallback untuk overlay

async function initItemLookupDisplay() {
    try {
        const res = await fetch('database/itemList.json?t=' + new Date().getTime());
        if (!res.ok) return;
        ...
        records.forEach(r => {
            ...
            if (item && item.equipid && item.equipname) {
                itemLookupDisplay[item.equipid] = item.equipname;
                // Simpan URL CDN sebagai fallback
                if (item.equipicon) {
                    nameToIconDisplay[item.equipname] = item.equipicon;
                }
            }
        });
    } catch (e) {
        // itemList tidak kritis untuk display — abaikan error
    }
}
initItemLookupDisplay();

// ============================================================
// HERO LOOKUP (untuk display overlay — safety net)
// Single source of truth: herolist.json kini berisi { id, name, img, voice }
// ============================================================
let heroIdLookupDisplay = {}; // { heroid: heroname }

async function initHeroIdLookupDisplay() {
    try {
        const res = await fetch('database/herolist.json?t=' + new Date().getTime());
        if (!res.ok) return;
        const list = await res.json();
        list.forEach(h => {
            if (h.id && h.name) heroIdLookupDisplay[h.id] = h.name;
        });
    } catch (e) {
        // Abaikan error — hero akan pakai nama dari matchdata
    }
}
initHeroIdLookupDisplay();

// Fungsi untuk mendapatkan path gambar
// Mendukung dua format item:
//   - String (nama file, contoh: "Winter Truncheon") → Assets/Itemandspell/Winter Truncheon.png
//   - Angka/equipid (contoh: 3207) → resolve dulu ke nama via itemLookupDisplay
function getImagePath(name, type) {
    if (!name || name === "idle" || name === "") return "";

    if (type === 'item') {
        // Cek apakah ini adalah equipid numerik
        const numId = parseInt(name);
        if (!isNaN(numId) && String(numId) === String(name)) {
            // Resolve ke nama via lookup, atau langsung pakai ID jika tidak ada
            const resolvedName = itemLookupDisplay[numId] || name;
            return `Assets/Itemandspell/${resolvedName}.png`;
        }
...
function renderPlayerList(players, containerId) {
    ...
    players.forEach(player => {
        ...
        // 4. Item Frame
        // Hybrid: lokal dulu, fallback CDN Moonton jika tidak ada
        function makeItemSlot(item) {
            if (!item || item === 'idle') {
                return `<div class="itemslot empty"></div>`;
            }
            const localSrc = getImagePath(item, 'item');
            // Cari CDN URL: dari nameToIconDisplay (equipname→icon)
            // Jika tidak ada (bisa terjadi saat lookup belum selesai load), biarkan kosong — onerror akan hide img
            const cdnUrl = nameToIconDisplay[item] || '';
            return `<div class="itemslot">
                        <img src="${localSrc}"
                             data-fallback="${cdnUrl}"
                             alt="${item}"
                             onerror="if(this.dataset.fallback){this.src=this.dataset.fallback;this.dataset.fallback=''}else{this.style.opacity='0'}">
                    </div>`;
        }

        const itemFrameHTML = `
            <div class="itemframe">
                <div class="items">${topItems.map(item => makeItemSlot(item)).join('')}</div>
                <div class="items">${botItems.map(item => makeItemSlot(item)).join('')}</div>
            </div>`;
...
function renderHeroCards(players, startImgId, startNameId) {
    players.forEach((player, index) => {
        ...
        if (imgElement) {
            // Coba p.hero dulu (nama dari herolist/matchdatateam.json)
            // Fallback: resolve dari heroid jika ada
            const heroName = player.hero ||
                             (player.heroid ? heroIdLookupDisplay[player.heroid] : null);
            if (heroName) {
                imgElement.src = getImagePath(heroName, 'hero');
