...
async function loadFromServer() {
    try {
        ...
        const coachBlueEl = document.getElementById('coach-input-blue');
        if (coachBlueEl) coachBlueEl.value = blue.coach || "";
        ...
        const coachRedEl = document.getElementById('coach-input-red');
        if (coachRedEl) coachRedEl.value = red.coach || "";
...
async function saveToServer() {
    try {
        ...
        const coachBlueInput = document.getElementById('coach-input-blue');
        fullData.teamdata.blueteam.coach = coachBlueInput ? coachBlueInput.value : "";
        ...
        const coachRedInput = document.getElementById('coach-input-red');
        fullData.teamdata.redteam.coach = coachRedInput ? coachRedInput.value : "";
...
function setupAutoSave() {
    ...
    for (let i = 1; i <= 14; i++) {
        ...
        if (input) input.addEventListener('input', saveToServer);
    }
    // Listener untuk Coach
    const coachBlue = document.getElementById('coach-input-blue');
    if (coachBlue) coachBlue.addEventListener('input', saveToServer);
    const coachRed = document.getElementById('coach-input-red');
    if (coachRed) coachRed.addEventListener('input', saveToServer);
...
async function resetNames() {
    ...
    const coachBlue = document.getElementById('coach-input-blue');
    if (coachBlue) coachBlue.value = "";
    const coachRed = document.getElementById('coach-input-red');
    if (coachRed) coachRed.value = "";
...
async function switchNames() {
    ...
    const coachBlueEl = document.getElementById('coach-input-blue');
    const coachRedEl = document.getElementById('coach-input-red');
    if (coachBlueEl && coachRedEl) {
        let blueCoach = coachBlueEl.value;
        coachBlueEl.value = coachRedEl.value;
        coachRedEl.value = blueCoach;
    }
