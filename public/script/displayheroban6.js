  ...
  ws.onmessage = (event) => {
    try {
      ...
      // 2. TANGKAP DETAK TIMER TERISOLASI DARI CONTROLLER (Tick / Detik)
        ...
        syncTimerTick(
          msg.data.draftdata.timer,
          msg.data.draftdata.timer_running,
          false,
        );
      }
...
function processData(newDraftData) {
  const prevPhase = currentDraftData
...
function updateMapDisplay(drawData) {
  ...
  if (drawData && drawData.result) {
    ...
    // Nama file di folder Assets/mapicon/ pakai kapital & spasi (e.g. "Expanding River.png")
    imgEl.src = `Assets/mapicon/${mapName}.png`;
    imgEl.alt = mapName;
    imgEl.onerror = function () {
      // Fallback: coba huruf kecil semua
  ...
  if (currentPhaseIndex < phasesActiveBoxes.length) {
    phasesActiveBoxes[currentPhaseIndex].forEach((boxId) => {
      ...
      if (phaseBox) {
        const isBanPhase =
          currentPhaseIndex < 4 ||
          (currentPhaseIndex >= 8 && currentPhaseIndex <= 9);
...
function updatePickAnimation(data, phaseIdx) {
  const safePickBlue =
    data.blueside && data.blueside.pick ? data.blueside.pick : [];
  const safePickRed =
    data.redside && data.redside.pick ? data.redside.pick : [];

  const activeSlots = pickPhaseToSlots[phaseIdx] || { blue: [], red: [] };

  const isBlueSlotFilled = (slot) =>
    !!(safePickBlue[slot - 1] && safePickBlue[slot - 1].hero);
  const isRedSlotFilled = (slot) =>
    !!(safePickRed[slot - 1] && safePickRed[slot - 1].hero);
