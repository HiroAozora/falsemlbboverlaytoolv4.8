...
// --- ITEM LIST API (Kamus equipid → equipname dari itemList.json) ---
const itemListPath = path.join(dbDir, 'itemList.json');
app.get('/api/itemlist', async (req, res) => {
    try {
        const data = await fs.readFile(itemListPath, 'utf8');
        res.json(JSON.parse(data));
    } catch (error) {
        console.error('Error reading itemList.json:', error);
        res.status(500).json({ message: 'Error reading item list' });
    }
});

// --- HERO ID API (Kamus heroid → heroname dari heroid.json) ---
const heroIdPath = path.join(dbDir, 'heroid.json');
app.get('/api/heroid', async (req, res) => {
    try {
        const data = await fs.readFile(heroIdPath, 'utf8');
        res.json(JSON.parse(data));
    } catch (error) {
        console.error('Error reading heroid.json:', error);
        res.status(500).json({ message: 'Error reading hero ID list' });
    }
});

// --- HERO LIST API (Kamus heroname → img path dari herolist.json) ---
const heroListPath = path.join(dbDir, 'herolist.json');
app.get('/api/herolist', async (req, res) => {
    try {
        const data = await fs.readFile(heroListPath, 'utf8');
        res.json(JSON.parse(data));
    } catch (error) {
        console.error('Error reading herolist.json:', error);
        res.status(500).json({ message: 'Error reading hero list' });
    }
});

// --- MOONTON MATCH API PROXY (CORS Bypass) ---
// Browser tidak bisa langsung fetch ke sg-api.mobilelegends.com karena CORS policy.
// Server Node.js ini bertindak sebagai proxy untuk meneruskan request.
app.get('/api/proxy-match', async (req, res) => {
    const { matchId } = req.query;
    if (!matchId) {
        return res.status(400).json({ code: 400, message: 'matchId is required' });
    }

    const targetUrl = `https://sg-api.mobilelegends.com/matchTools/v1/getMatchUrl?matchId=${matchId}`;

    try {
        // Gunakan dynamic import untuk 'node-fetch' atau gunakan http native sebagai alternatif
        // Node.js 18+ mendukung fetch() secara native
        const fetchFn = globalThis.fetch || (await import('node-fetch').then(m => m.default).catch(() => null));

        if (!fetchFn) {
            // Fallback menggunakan https bawaan Node.js jika fetch tidak tersedia
            ...
            const proxyReq = https.request(options, (proxyRes) => {
                ...
                proxyRes.on('end', () => {
                    try {
                        ...
                        res.status(502).json({ code: 502, message: 'Invalid JSON from Moonton API', raw: body.substring(0, 200) });
                    }
                });
            });
            proxyReq.on('error', (e) => {
                console.error('Proxy request error:', e.message);
                res.status(503).json({ code: 503, message: 'Cannot connect to Moonton API', detail: e.message });
            });
            proxyReq.end();
            return;
        }

        // Path menggunakan native fetch (Node 18+)
        const apiResponse = await fetchFn(targetUrl, {
            headers: {
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36',
                'Accept': 'application/json',
                'Referer': 'https://m.mobilelegends.com/'
        ...
        if (!apiResponse.ok) {
            return res.status(apiResponse.status).json({
                ...
                message: `Moonton API responded with status ${apiResponse.status}`
            });
        }
        ...
        console.error('Proxy-match error:', error.message);
        res.status(503).json({ code: 503, message: 'Cannot connect to Moonton API', detail: error.message });
    }
});
